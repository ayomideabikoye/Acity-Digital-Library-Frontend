  ...
  return `
        <div class="book-card" data-book-id="${book.book_id}">
            <div>
                ...
                <p><strong>Available Copies:</strong> <span class="${
                  isAvailable ? "available" : "unavailable"
                }">${book.available_copies}</span> / ${book.total_copies}</p>
            </div>
            <div class="book-actions">
                ${adminActions}
                <button onclick="window.handleBorrow(${
                  book.book_id
...
window.fetchBooks = async () => {
  try {
    ...
    const books = await fetchAPI(
      `/api/books?search=${search}&category=${category}`,
      "GET",
      null,
      true
    );
    ...
    if (books.length === 0) {
      bookList.innerHTML =
        '<p style="text-align: center; color: var(--text-muted); padding: 50px;">No books found matching your criteria.</p>';
      return;
    }
...
window.handleAddBook = async (event) => {
  ...
  if (!title || !isbn || !total_copies || !category) {
    return window.displayMessage(
      "message-admin",
      "Missing Title, ISBN, Total Copies, or Category.",
      "error"
    );
  }
...
window.handleBorrow = async (bookId) => {
  try {
    ...
    displayMessage(
      "message-catalogue",
      `Book borrowed successfully! It is due in 15 days.`,
      "success"
    );
    window.fetchBooks();
  } catch (error) {
...
window.fetchMyBooks = async () => {
  try {
    const transactions = await window.fetchAPI(
      "/api/transactions/my-books",
      "GET"
    );
    ...
    if (transactions.length === 0) {
      listEl.innerHTML =
        '<p style="text-align:center; color:#666; padding: 20px;">You have no borrowed or returned books in your history.</p>';
      return;
    }
    ...
    listEl.innerHTML = transactions
      .map((t) => {
        ...
        const returnDateDisplay = t.return_date
